...
local function on_blame_done(lines)
  ...
  -- Keymaps
  au.exec_useraucmd(au.user_autocmds.on_git_blame_done)
  vim.api.nvim_command("autocmd BufWinLeave <buffer> lua require('userlib.git.blame').blame_quit()")
...
local function on_blame_commit_done(commit_hash, lines)
  ...
  vim.api.nvim_command("autocmd BufLeave <buffer> lua require('userlib.git.blame').blame_commit_quit()")
...
function M.blame_commit()
  ...
  local function on_event(_, data, event)
    ...
    if event == 'stdout' or event == 'stderr' then
      ...
      for i = 1, #data do
        if data[i] ~= '' then table.insert(lines, data[i]) end
      end
    end
